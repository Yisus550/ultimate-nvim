...
-- Note: Java (jdtls) is configured separately via after/ftplugin/java.lua
-- using mfussenegger/nvim-jdtls plugin for per-project workspace management

-- Disable conflicting/duplicate servers
-- ts_ls conflicts with vtsls (both are TypeScript servers)
vim.lsp.config("ts_ls", { enabled = false })
vim.lsp.config("tsserver", { enabled = false })

-- Simple LSP servers: Enable them if executable is found
...
for server_name, lsp_executable in pairs(enabled_lsp_servers) do
  if utils.executable(lsp_executable) then
    ...
    -- Check if this is a Mason-managed server that might still be installing
    local mason_registry_ok, mason_registry = pcall(require, "mason-registry")
    if mason_registry_ok and mason_registry.is_installed(server_name) then
      -- Server is installed via Mason but not in PATH yet, enable it anyway
      vim.lsp.enable(server_name)
    else
      local msg = string.format(
        "Executable '%s' for server '%s' not found! Server will not be enabled. Run :Mason to install it.",
        lsp_executable,
        server_name
      )
